...
│       ├── train.py                 # LightGBM, EBM Primary, EBM Distilled, RuleFit trainers
│       └── walk_forward.py          # Walk-forward CV splitter
├── notes/                           # SHAP plots and figures (generated by pipeline scripts)
│   ├── pipeline.png
│   ├── shap_summary.png
...
| GMM | 2 regimes (Bull/Bear) ordered by volatility, fit on train only |
| EBM Primary | Hard labels {0,1}, regularized: max_rounds=1500, interactions=3, min_samples_leaf=20 |
...
**Key findings:**
1. OOS AUC ~0.487–0.499 — typical for equity markets without strong macro signals; no model beats noise threshold
